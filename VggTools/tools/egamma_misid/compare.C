void compare(int year, bool isQCD, string filename, string syst) {

   string sqcd = "";
   if (isQCD) sqcd = "_qcd";

   string CB_title = "../../macros/html/egamma_v5/" + syst + "/sf_egamma_misid_" + filename + "_CB" + sqcd + "_";
   CB_title += std::to_string(year);
   CB_title = CB_title + ".root";

   string TP_title = "../../macros/html/egamma_v5/" + syst + "/sf_egamma_misid_" + filename + "_TP" + sqcd + "_";
   TP_title += std::to_string(year);
   TP_title = TP_title + ".root";

   TFile * f_CB = new TFile((CB_title).c_str());
   TH2D * h_CB = (TH2D*)f_CB->Get("EGamma_SF2D");

   TFile * f_TP = new TFile((TP_title).c_str());
   TH2D * h_TP = (TH2D*)f_TP->Get("EGamma_SF2D");

   TH2D* h_ratio = (TH2D*)h_CB->Clone("h_ratio");
   h_ratio->Divide(h_TP);

   TH2D* h_ratio_err = (TH2D*)h_CB->Clone("h_ratio_err");
   ...
   for(int i = 0; i <= h_ratio_err->GetNbinsX(); i++) {
     for(int j = 0; j <= h_ratio_err->GetNbinsY(); j++) {
       h_ratio_err->SetBinContent(i, j, h_CB->GetBinError(i,j) / h_TP->GetBinError(i,j));
     }
   }

   TCanvas * c1 = new TCanvas("c1","c1",0,0,1000,1000);
   c1->cd();
   h_ratio->SetTitle("Ratio CB/TP");
   h_ratio->Draw("COLZ TEXT");

   TCanvas * c2 = new TCanvas("c2","c2",0,0,1000,1000);
   c2->cd();
   h_ratio_err->SetTitle("Error ratio CB/TP");
   ...
   TH2D* h_final = (TH2D*)h_TP->Clone("h_final");
   ...
   for(int j = 1; j <= h_final->GetNbinsY(); j++) {
     for(int i = 1; i <= h_final->GetNbinsX(); i++) {
       float bin_err = TMath::Sqrt(TMath::Power(h_final->GetBinError(i,j), 2) + TMath::Power(fabs(h_TP->GetBinContent(i,j) - h_CB->GetBinContent(i,j)), 2)); 
       final_txt << std::fixed << std::setprecision(2) << h_final->GetBinContent(i,j) << "\t\t" << bin_err << "\t\t" << std::setprecision(1) << bin_err/h_final->GetBinContent(i,j)*100 << " \%" << std::setprecision(2) << "\t\t" << h_final->GetBinError(i,j) << "\t\t" << fabs(h_TP->GetBinContent(i,j) - h_CB->GetBinContent(i,j)) << endl;
