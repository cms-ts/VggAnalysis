...
void plot3(string plot="", string title="", string version="", string flags="") {
  ...
  if (plot.find("Zgg") != string::npos) index = 11;
  if (plot.find("Wgg") != string::npos) index = 1011;

  for (int i = 0; i < histo2[0]->GetNbinsY(); i++) {
    double s0 = -1;
    double s1 = -1;
    double s2 = -1;
    double s3 = -1;
    ...
    double s0_b = histo2[index]->GetBinContent(1, i);
    ...
    double s0_e = histo2[index]->GetBinContent(1+4, i);
    double s1_e = histo2[index]->GetBinContent(2+4, i);
    double s2_e = histo2[index]->GetBinContent(3+4, i);
    double s3_e = histo2[index]->GetBinContent(4+4, i);
  ...
  TPad* pad1_c1 = new TPad("pad1_c1", "pad1_c1", 0.0, 0.3, 1.0, 1.0);
  pad1_c1->SetBottomMargin(0.001);
  pad1_c1->Draw();
  pad1_c1->cd();
  ...
  TPad* pad2_c1 = new TPad("pad2_c1", "pad2_c1", 0.0, 0.0, 1.0, 0.3);
  pad2_c1->SetTopMargin(0);
  pad2_c1->SetBottomMargin(0.3);
  pad2_c1->Draw();
  pad2_c1->cd();
  ...
  TFile* file1 = new TFile(("html/" + version + "/" + year + ".matrix/root/matrix_weight.root").c_str(), "update");
