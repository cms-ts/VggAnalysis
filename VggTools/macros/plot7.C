...
int plot7(string plot="", string title="", string version="v00", string options="") {
  ...
  if (plotMap.size() == 0) {
    Error("plot7", "plot map %s is EMPTY or MISSING !!", plot.c_str());
    return 1;
  }
  ...
  for (multimap<string, double>::iterator it = plotMap.begin(); it != plotMap.end(); it++) {
    ...
    if (index == 0) {
      if (lumiMap[it->first] != 0) {
        ...
        Warning("plot7", "luminosity for %s is ZERO !!", it->first.c_str());
      }
    }
  }

  if (lumi == 0) {
    Error("plot7", "total luminosity is ZERO !!");
    return 1;
  }
...
return plot7(argv[1], argv[2], argv[3], argv[4]);
