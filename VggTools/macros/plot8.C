...
void plot8(string plot="", string title="", string version="v00", string options="", int nfits=2) {
  ...
  if (file1.is_open()) {
    file1 >> fitval >> fiterr;
    file1.close();
    TFile* file2 = new TFile(("html/" + version + "/reference/" + year + ".qcd/root/" + title + "_qcd_nofit.root").c_str());
    if (!file2->IsZombie()) {
