...
int plot8(string plot="", string title="", string version="v00", string options="default", int nfits=2) {
  ...
  if (plot.find("Zgg") != string::npos) {
    if (options.find("qcd") != string::npos) {
      Warning("plot8", "skipping plot map %s for options %s", plot.c_str(), options.c_str());
      return 0;
    }
  }
  ...
  if (plotMap.size() == 0) {
    Error("plot8", "plot map %s is EMPTY or MISSING !!", plot.c_str());
    return 1;
  }
  ...
  for (multimap<string, double>::iterator it = plotMap.begin(); it != plotMap.end(); it++) {
    ...
    if (index == 0) {
      ...
      if (lumiMap[it->first] != 0) {
        ...
        Warning("plot8", "luminosity for %s is ZERO !!", it->first.c_str());
      }
      if (histo[index]) {
  ...
  if (lumi == 0) {
    Error("plot8", "total luminosity is ZERO !!");
    return 1;
  }
  ...
  if (title.find("h_W_") != string::npos) {
    file1.open(("html/" + version + "/reference/" + year + ".qcd/root/" + "h_W_" + title.substr(4, 3) + "_qcd_fit.dat").c_str());
  }
  if (title.find("h_WG_") != string::npos) {
    file1.open(("html/" + version + "/reference/" + year + ".qcd/root/" + "h_WG_" + title.substr(5, 3) + "_qcd_fit.dat").c_str());
  }
  if (title.find("h_WGG_") != string::npos) {
    file1.open(("html/" + version + "/reference/" + year + ".qcd/root/" + "h_WG_" + title.substr(6, 3) + "_qcd_fit.dat").c_str());
  }

  if (file1.is_open()) {
    file1 >> fitval >> fiterr;
    file1.close();
    TFile* file = new TFile(("html/" + version + "/reference/" + year + ".qcd/root/" + title + "_qcd_nofit.root").c_str());
    if (!file->IsZombie()) {
  ...
  if (title.find("h_WG") != string::npos){
    myindex1 = 1011;
    if (nfits > 1) myindex2 = 1012;
  }
  if (title.find("h_ZG") != string::npos){
    myindex1 = 11;
    if (nfits > 1) myindex2 = 12;
  }

  h_data_fit = (TH1D*)histo[0]->Clone("h_data_fit");
  h_mc_fit0 = (TH1D*)histo[myindex1]->Clone("h_mc_fit0");
  if (myindex2 != -1) h_mc_fit1 = (TH1D*)histo[myindex2]->Clone("h_mc_fit1");

  for (map<int, TH1D*>::iterator it = histo.begin(); it != histo.end(); it++) {
    int index = int(it->first);
    if (index != 0 && index != myindex1 && index != myindex2 && histo[index]) {
      h_data_fit->Add(histo[index], -1);
    }
  }
  ...
  cout << "" << endl;
  cout << "+++++++++++++++++++++++++++++++++++" << endl;
  cout << "Single photon = " << fitter->GetParameter(0) << " +- " << fitter->GetParError(0) << endl;
  cout << "Inclusive     = " << fitter->GetParameter(1) << " +- " << fitter->GetParError(1) << endl;

  double chi2, edm, errdef;
  int nvpar, nparx;
  fitter->GetStats(chi2,edm,errdef,nvpar,nparx);

  cout << "chiq = " << chi2/(double)(h_data_fit->GetNbinsX() - 2) << endl;
  cout << "+++++++++++++++++++++++++++++++++++" << endl;
  cout << "" << endl;

  histo[myindex1]->Scale(fitter->GetParameter(0));
  if (myindex2 != -1) histo[myindex2]->Scale(fitter->GetParameter(1));
  ...
  for (map<int, TH1D*>::iterator it = histo.begin(); it != histo.end(); it++) {
    ...
    if (index == 12) {
      ...
      legend->AddEntry(histo[index], "DYJets", "f");
    }
    if (index == 13) {
    ...
    if (index == 20) {
      ...
      legend->AddEntry(histo[index], "WJets", "f");
    }
    if (index == 21) {
    ...
    if (index == 30) {
      ...
      legend->AddEntry(histo[index], "Diboson", "f");
    }
    if (index == 31) {
      histo[index]->SetFillColor(kGreen+3);
      legend->AddEntry(histo[index], "Diboson #gamma", "f");
    }
    if (index == 35) {
      histo[index]->SetFillColor(kGreen+2);
      legend->AddEntry(histo[index], "Triboson", "f");
    }
    if (index == 40) {
    ...
    if (index == 50) {
      ...
      legend->AddEntry(histo[index], "SingleTop", "f");
    }
    if (index == 51) {
      histo[index]->SetFillColor(kOrange+7);
      legend->AddEntry(histo[index], "SingleTop #gamma", "f");
    }
